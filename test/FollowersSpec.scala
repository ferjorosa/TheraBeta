...
//These test rely on each other. if one the above fails the others behind that will probably fail
...
class FollowersSpec extends CustomSpec{
  ...
  "The Follower Persistence Layer" should "be able to insert new followers" in{
    ...
    val insertionSequence = Future.sequence(List(
      Followed.insertNewFollowed(Follower(deviceA,deviceB)),
      Followed.insertNewFollowed(Follower(deviceA,deviceC)),
      Followed.insertNewFollowed(Follower(deviceB,deviceC))
    ))
  ...
  it should "be able to insert new followings" in{
    ...
    // (A -> B) ; (A -> C) ; (B -> C)
    val insertionSequence = Future.sequence(List(
      Following.insertNewFollowing(Follower(deviceA,deviceB)),
      Following.insertNewFollowing(Follower(deviceA,deviceC)),
      Following.insertNewFollowing(Follower(deviceB,deviceC))
    ))
    //Synchronous (only for unit testing))
    Await.ready(insertionSequence, 5 seconds).onFailure{case t => fail("Fail to insert rows in the Followed Table")}
  }
  ...
  it should "be able to get the device's followers" in{
    Followed.getFollowersOfDevice(deviceC) onComplete{
  ...
  it should "be able to get the device's followings" in{
    Following.getFollowingsOfDevice(deviceA) onComplete{
  ...
  it should "be able to delete a follower relation" in{
    val delete = Followed.deleteFollowed(Follower(deviceA,deviceC))
  ...
  it should "be able to delete a following relation" in {
    val delete = Following.deleteFollowing(Follower(deviceA,deviceC))
